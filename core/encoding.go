...
func (enc *GobBlockEncoder) Encode(block *Block) error {
	return gob.NewEncoder(enc.w).Encode(block)
}
...
func (dec *GobBlockDecoder) Decode(block *Block) error {
	return gob.NewDecoder(dec.r).Decode(block)
}
